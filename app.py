...
def build_thread_tree(items, root_id, max_replies=24):
    ...
    # sort by timestamp so we attach in chronological order
    ...
    for item in items_sorted:
        ...
        if tid == root_id:
            continue
        # only attach if parent node exists and we haven't exceeded replies
        if pid in nodes and count < max_replies:
            nodes[pid]["children"].append(nodes[tid])
            count += 1

    # return the subtree under root
...
def main():
    ...
    # fetch up to root + 24 replies = 25 total
