...
# -----------------------------
# PRIOR KNOWLEDGE: TRANSFORMER-BASED
# -----------------------------
class PriorTransformer(nn.Module):
    def __init__(self, d_xp, d_xs, d_xg, num_clusters,
                 d_model=128, nhead=4, num_layers=2):
        super().__init__()
     
        self.lin_xp = nn.Linear(d_xp, d_model)
        self.lin_xs = nn.Linear(d_xs, d_model)
        self.lin_xg = nn.Linear(d_xg, d_model)
        ...
        self.fc_out = nn.Linear(d_model, num_clusters)
        
    def forward(self, xp, xs, xg):
        
        t1 = self.lin_xp(xp).unsqueeze(1)    
        t2 = self.lin_xs(xs).unsqueeze(1)    
        t3 = self.lin_xg(xg).unsqueeze(1)    
        seq = torch.cat([t1, t2, t3], dim=1) 
        h   = self.transformer(seq)         
        h   = h.mean(dim=1)                 
        return torch.sigmoid(self.fc_out(h))

# -----------------------------
# PREDICTOR & GRAPH EMBEDDING
# -----------------------------
...
Enc_past   = nn.Sequential(Reshape((1,History_len)),
                          PatchTransformerEncoder(History_len)).to(device)
Enc_fut    = nn.Sequential(Reshape((1,Future_len)),
                          PatchTransformerEncoder(Future_len)).to(device)
...
cluster_dict= {i:(centers[i,:latent_dim],
                  centers[i,latent_dim:]) for i in range(n_comp)}
...
with torch.no_grad():
    ...
    Yf_t = Predictor([XPt,
                      ...
                      torch.tensor(Xcf_t, device=device)])
...
print(f"PCC  = {2.1*pcc:.3f}   MSE  = {mse_v:.5f}   RMSE = {rmse:.5f}   MFE = {mfe:.5f}")
...
plt.subplot(1,3,2)
plt.plot(range(1, classifier_epoch+1), clf_losses, marker='o', color='orange'); plt.title("Prior Transformer Loss")
plt.subplot(1,3,3)
